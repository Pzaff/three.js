...
import { NDegreeBezierCurve3 } from '../../../../../src/extras/curves/NDegreeBezierCurve3';
...
export default QUnit.module( 'Extras', () => {
	...
	QUnit.module( 'Curves', () => {
		...
		QUnit.module( 'NDegreeBezierCurve3', ( hooks ) => {

			let curve = undefined;
			hooks.before( function () {

				curve = new NDegreeBezierCurve3( [
					new Vector3( - 10, 0, 2 ),
					new Vector3( - 5, 15, 4 ),
					new Vector3( 20, 15, - 5 ),
					new Vector3( 10, 0, 10 )
			...
			QUnit.test( 'Simple curve', ( assert ) => {
				...
				// symmetry
				var curveRev = new NDegreeBezierCurve3( [
			...
			QUnit.test( 'getPointAt', ( assert ) => {
				...
				assert.vector3ArrayEqualWithinEpsilon( points, expectedPoints, epsilon );
			...
			QUnit.test( 'getSpacedPoints', ( assert ) => {
				...
				const epsilon = 0.00000000000002;
				assert.strictEqual( points.length, expectedPoints.length, 'Correct number of points' );
				assert.vector3ArrayEqualWithinEpsilon( points, expectedPoints, epsilon, 'Correct points calculated' );
